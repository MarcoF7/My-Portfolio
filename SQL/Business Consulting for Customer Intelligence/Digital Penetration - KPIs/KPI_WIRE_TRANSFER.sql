/*
KPI5. WIRE TRANSFERS
CREATION OF A TABLE TO SHOW THE NUMBER OF WIRE TRANSFERS AND TOTAL AMOUNT BROKEN OUT BY:
...
CREATE TABLE WIRE_TRANSFER AS 
SELECT * FROM (
SELECT 
	MONTH(T1.DAT ACCT)            AS BATCH_MONTH,
	'WIRE TRANSFER'               AS ITEM,
	...
	COUNT(T1.ACC)                 AS UNITS(#TRANSACTIONS),                                                                                    -- COUNTING EVERY TRANSACTION WOULD BE THE SAME AS COUNTING THE NUMBER OF OPERATIONS
	T1.OCC                        AS CURRENCY,
...
WHERE T1.ACCT BETWEEN 2017-11-01 AND 2018-10-31 /*MARCO: It should be DAT ACCT*/
        ...
        AND ((T1.CODE IN ('376') AND T1.TRANCODE IN ('0690','0750'))
        OR (T1.CODE IN ('525') AND T1.TRANCODE IN ('0719','0725', '748'))
        OR (T1.CODE IN ('D14') AND T1.TRANCODE IN ('746'))
        OR (T1.CODE IN ('527') AND T1.TRANCODE IN ('980'))
        OR (T1.CODE IN ('D51') AND T1.TRANCODE IN ('492', '532'))
        OR (T1.CODE IN ('D64') AND T1.TRANCODE IN ('503'))
        OR (T1.CODE IN ('196') AND T1.TRANCODE IN ('985', '986')))     
