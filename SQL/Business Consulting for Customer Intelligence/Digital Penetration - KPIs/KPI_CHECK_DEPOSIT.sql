/*
KPI3. CHECK DEPOSIT
CREATION OF A TABLE TO SHOW THE NUMBER OF CHECK DEPOSITS TRANSACTIONS AND TOTAL AMOUNT BROKEN OUT BY:
...
CREATE TABLE CHECK_DEPOSIT AS
SELECT * FROM (
SELECT 
	MONTH(T1.DAT ACCT)            AS BATCH_MONTH,
	'CHECK DEPOSIT'               AS ITEM,
	...
	COUNT(T1.NUM OPERATION)       AS UNITS(#TRANSACTIONS),                                                                                  -- COUNTING EVERY TRANSACTION WOULD BE THE SAME AS COUNTING THE NUMBER OF OPERATIONS PER ACCOUNT
	T1.OCC                        AS CURRENCY
...
-- T812. TABLE WITH CONSUMER/NON CONSUMER FLAG:	
  LEFT JOIN T812 T5
	 	ON T4.SYSTEM TYPE = T5.KEY TABLE, /*MARCO: T4.SYSTEM TYPE should be T4.SYSTEMTYPE*/                                                                                                     -- CONFIRM SYSTEM TYPE
